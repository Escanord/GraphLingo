...
def get_exact_matching_facts(args, storage, top_attributes, triples):
    ...
    edges = []
    nodes_checker = {}
    ids = get_exact_matching_nodes(args, triples, top_attributes)
    # gather info around exactly matched nodes
    for id in ids:
        sub_graph = get_k_hops_graph(args, id, 1)
        ...
        for triple in sub_graph:
            ...
            properties = get_triple_properties(args, triple['id(n)'], triple['id(r)'], triple['id(m)'])
    ...
    #BANKS algo
    lca_id, path_len = find_lca(args, ids)
    if lca_id is not None:
        for id in ids:
            if id != lca_id:
                path = get_path(args, lca_id, id, path_len)
                if path is not None:
                    for i, element in enumerate(path):
                        if i % 2 == 1:
                            update_triples_info(facts, [{'properties(n)': element[0], 'type(r)': element[1], 'properties(m)': element[2]}])
                            
                    path_ids = get_path_ids(args, lca_id, id, path_len)
                    for i, element in enumerate(path_ids):
                        if i % 2 == 1:
                            ...
                            properties = get_triple_properties(args, path_ids[i - 1], path_ids[i], path_ids[i + 1])
